...
wasmrt is licensed **`MIT OR Apache-2.0`**. That covers *our own* original code.
...
license compliance. (Mirrors the source project `wazmrt`'s compliance scaffold.)
...
> needs one.
>
> **wasmrt status:** 100% original Rust. The port of `wazmrt` is the owner's own
> code (not a third-party adoption). **Nothing is currently adopted** — the
> Component Ledger is empty. In particular, wasmrt ships its **own** `wasmrt.h`
> and does **not** vendor the standard wasm-c-api `wasm.h` (unlike wazmrt).
...
Apache-2.0 code cannot be relabeled as MIT. Because wasmrt is dual `MIT OR
Apache-2.0`, incorporated Apache-2.0 code keeps its Apache terms for those files
— a downstream user who chooses "MIT" for wasmrt still complies with Apache-2.0
...
3. In any source file where we adapt Apache-2.0 code, add a header change-note:
   `// Adapted from <project> (<SPDX>); modified by wasmrt — see third_party/LICENSES.md`.
...
Newest first. **Empty** — wasmrt has adopted no third-party code. Copy the
template below for each future adopted component.
...
- **Where it lives in wasmrt:** <path(s)>
...
> **Note on `wasm.h`:** the source project wazmrt vendored the standard
> wasm-c-api `wasm.h` (Apache-2.0) because its integration ABI *was* the
> wasm-c-api. wasmrt deliberately ships its **own** `wasmrt.h` instead (see
> `cmem/loaders.md`), so it vendors nothing and has no such ledger entry.

---

## Reference project inventory

Evaluation candidates (same set as wazmrt). Status is **Evaluating** — code is
adopted only after an Adoption Checklist + Component Ledger entry above.
Verified against each upstream `LICENSE` file (per wazmrt, 2026-07-02).
**wasmtime is additionally the feature-parity target** ("run what wasmtime runs";
see `cmem/vision.md`) and the runtime wasmrt replaces under the loaders — that is
a behavioral goal, not a code adoption.

| Project | License (SPDX) | Status |
|---|---|---|
| [wasm-micro-runtime (WAMR)](https://github.com/bytecodealliance/wasm-micro-runtime) | `Apache-2.0 WITH LLVM-exception` | Evaluating (size benchmark) |
| [wasm3](https://github.com/wasm3/wasm3) | `MIT` | Evaluating (size benchmark) |
| [wasmtime](https://github.com/bytecodealliance/wasmtime) | `Apache-2.0 WITH LLVM-exception` | Evaluating (feature-parity target; being replaced under the loaders) |
...
| [wasm-c-api](https://github.com/WebAssembly/wasm-c-api) (the C API standard) | `Apache-2.0` | Not adopted — wasmrt ships its own `wasmrt.h` |

> **Trademarks:** permissive licenses grant no trademark rights. Do not use the
> "Wasmtime", "Wasmer", "wazero", etc. names to brand wasmrt or imply

...
> **Status: early — `0.6.5`.** wasmrt can now **decode**, **type-check**, and **run** a WebAssembly
> module — `wasmrt run fac.wasm fac 10` → `3628800`. Execution currently covers integer + floating-point
> compute, linear memory, tables / `call_indirect` / reference types, **WasmGC** (structs, arrays, `i31`,
> casts), and **SIMD** (the full `v128` set, incl. relaxed); threads, memory64, and the rest land slice by
> slice. See **[ROADMAP.md](ROADMAP.md)** for the live use-case matrix (what actually works today) and

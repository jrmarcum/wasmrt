...
CONFORMANCE CLEAR-OUT (`1.0.0`) — IS IN PROGRESS, day 4 landed 2026-09-19 (track A done as the custom-annotations feature; the shipped cdylib −62% after `rlib` was found defeating LTO; the import type-use check #4 closed; TRACK P custom-page-sizes done — `PAGE_SIZE` deleted, byte-granular bounds cross-checked on wasmtime; 🎯 **TRACK D (D1–D4) done — the corpus is at ZERO failed / ZERO skipped / ZERO unrun**); day 5 was the DECODER review and 🆕 **day 6 (2026-09-20) the VALIDATOR review — eight findings, two of them silent wrong answers that EXECUTED**, all unreleased.**
wasmrt assembles, decodes, validates, runs, does WASI preview 1 with a sandboxed filesystem, and is
**embeddable from C** via `wasmrt.h`. Spec suite ✅ **64,603 / 0 / 0** over **288** files (per-file feature sets, `wast::features_for_script`) — **SETTLED 2026-09-19 late**, after wasmtk finished: their fix went into their RUNNER and the vendored patch was narrowed, restoring 5 × "multiple memories" because upstream carries them. Those 5 were ours to answer: 🔒 **a proposal snapshot must not be given features that POSTDATE it**, so `proposals/threads/` now runs with multi-memory OFF. Measured, not assumed — wasmtime fails the same directive with multi-memory on, and disabling it suite-wide breaks five core files, so nothing is hidden (`cmem/testing.md`, top), **589 workspace tests**, clippy clean, C-ABI gate PASSED, `.wat` corpus **528 / 532 files** — 🆕 **and every one of the 528 is now accepted by `wasm-tools`, not just by us** (`scripts/wat-corpus.ts` ends the gate at an OUTSIDE READER; the 4 are corpus defects wasm-tools refuses on the same lines) ⚠️ *(an earlier reading of this gate keyed on an exit status that is always 0, so two pre-existing failures were invisible)*, no file lost a TRUE pass in any pass (day 4 removed one false one — `cmem/roadmap.md`, DAY 4). ✅ **Miri 32/32 PASSED (day 4, 2026-09-19)** — re-count it every run; it was 28 on record while the crate had 31.
...
🔴 **DAY 5 (2026-09-19) — A DECODER REVIEW FOUND EIGHT DEFECTS, AND FIXING ONE FOUND A NINTH IN THE ASSEMBLER.** All eight reproduced against wasm-tools 1.259 **before** anything changed (the standing *prove, don't assume* rule), and **two were the opposite direction from how they were filed**: a private `skip_const_expr` — a **second, partial copy of the instruction grammar** — REFUSED TWO VALID MODULES (`struct.new_desc`; `ref.null` with an `exact` heap type). ⚠️ Both were benign at type index 0 and fatal at 11, where the stranded immediate byte IS `0x0B` = `end` — **a probe that picks the benign input reports "no defect" in the same words as a correct implementation.** It is deleted: const exprs now go through `opcode::decode_expr`, the reader that decodes function bodies. The other six were **fields whose undefined values were read and discarded** — `br_on_cast` flags, `atomic.fence`'s reserved byte, the elemkind byte, element flags `8` (which decoded as a DIFFERENT segment form), **20 unassigned SIMD sub-opcodes** (🎓 *a ceiling is not membership*), and `array.new_data`/`array.init_data` missing from the data-count requirement. ⚠️⚠️ **That last fix made one of our OWN tests fail, which is how the ninth surfaced: the ASSEMBLER had the identical gap, so `wasmrt wat` emitted modules `wasm-tools` refuses** — the seventh instance of the T10a emitter mechanism, and the second where our assembler's output is not WebAssembly. 🔒 The response is a gate: `scripts/wat-corpus.ts`, proved to fail by reverting both fixes. Conformance **unchanged at 64,603 / 0 / 0** — none of this is reachable from the spec suite. `cmem/known-issues.md` (top), `cmem/best-practices.md` §1.9 / §3.13 / §3.8d / §8.1b.
🔴 **DAY 6 (2026-09-20) — A VALIDATOR REVIEW FOUND EIGHT, AND TWO OF THEM WERE NOT IN THE VALIDATOR.**
All reproduced against wasm-tools 1.259 and wasmtime 48 first. 🔴 **A table's ELEMENT TYPE was read as
a VALUE type** (§5.3.9 says *reftype*), so `(table (elem i64))` decoded, validated and **RAN** —
`table.get` handed the guest the engine's null sentinel as an `i64` — and **`wasmrt wat` emitted such
modules**: the eighth T10a emitter instance, the third time our assembler's output is not WebAssembly.
🎓 The element SEGMENT's field is the *same grammar production* and had been fixed on its own, because
that is the one the suite complained about (§1.10). 🔴 **Every 64-bit TABLE access truncated its index
to 32 bits**: `call_indirect` at `2^32` **called slot 0 and returned its answer** where wasmtime traps
— six instructions, and `Table::is64` was already recorded *for import matching* with nothing on the
execution path reading it (§3.14). Then: `return_call_indirect` typed its table index `i32` (both
directions); a constant expression's operand stack was capped at **8**, refusing valid modules with the
wrong reason; an uninitialized non-null local was readable in unreachable code; **table64 and the
table-initializer form were each gated at one entry point of two** (X3, third and fourth instances);
export-name uniqueness was **O(n²)** (**128k exports = 11.9 s** vs wasm-tools' 0.13 s — a DoS before
anything runs); and **`--features` was applied by ONE CLI path of three**, a live swappability break
(`interop.md` §2.2f; the sibling refuses the same command line). 🎓 **Three of the eight came from
sweeps and one from a fuzz, not from reading the arm that was wrong** — the suite stayed at
64,603 / 0 / 0 the whole time, because it cannot spell `(table 1 i64)` and cannot test a gate while
every proposal is on (§1.11, §1.12). The three sweeps are committed as gates:
`scripts/feature-gate-sweep.ts`, `scripts/index-type-sweep.ts`, `scripts/differential-fuzz.ts`.

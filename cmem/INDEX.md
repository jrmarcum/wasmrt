...
**Then two more tasks stand before 1.0** (owner, 2026-08-06): **T10 (0.11.0) — bug hunt + code hygiene**
(the "look for code issues" trigger below, run as a scheduled task: bugs, fall-throughs, stale
workarounds, dead code, missing docs) and **T11 (0.12.0) — optimization review**, which is a
**discussion with measurements, not a pass**, judged at the binary and C-ABI boundary. The order
**measure (T9) → find (T10) → optimize (T11)** is deliberate: optimizing before the bug hunt tunes code
that may be about to change, and reviewing performance before T9 produces a baseline yields opinions
rather than deltas.
...
| [roadmap.md](roadmap.md) | **T9 is scoped from a measured audit (2026-08-06)** — T9a correctness punch-list (12 items, each with its measured cost, ordered by value), T9b size, T9c perf, T9d licensing/docs, T9e `pin`, T9f tail calls (the last unimplemented IN-SCOPE proposal — 1.0 cannot be claimed without them), T9g scope confirmations (what to leave failing, and why the in-scope residual is far smaller than 738/3,075 suggests). **T10 (0.11) = bug hunt + code hygiene** and **T11 (0.12) = optimization review** were added by the owner 2026-08-06; the ordering **measure → find → optimize** is deliberate. Current status (**PORT phase; T0–T8 done, published through v0.9.0, oracle frozen @dadc727**) + the **conversion task list** T0–T11 (scaffold → types/reader → opcode → decode → validate → interp slices → text → wasi → C-ABI → hardening → bug hunt → optimization review), parity-gated, with the 4 decision-gates inlined (**all now resolved**) and T9 scoped to what actually remains. Loader phases 1–4 |
| [known-issues.md](known-issues.md) | Issue tracker — 🔴 **2 NEW open defects from the T9 scoping audit (2026-08-06)**: `Op::MemorySize` reads **another instance's** memory (silent wrong output; the 4th and last of the shared-store class) and the `.wast` runner **redirects a failed module's assertions to an unrelated module** (misleading diagnostics). Plus: **`ref.null ` severity RAISED** — it costs **161 skipped assertions in `br_table.wast` alone**, not the 2 files originally logged. the **2 silent-wrong-output defects fixed at T8** (dropped table initializers; element-segment form 4 rewriting a type) **and the third-order lesson**: a check that is harmless "because the case cannot occur" becomes a bug the moment it can; what T8 closed and what it left open; the **ranked pre-T8 punch list** (2026-08-05), the **5 open defects the wasmtk corpus surfaced** (`ref.null $T` unassemblable · 2 validator gaps the oracle passes · no trap backtrace · an unconditional `data_count` section), the fixed shared-store and literal/text defect write-ups, the port's intentional divergences (single-threaded atomics, `delegate` rejected everywhere, the debug-build stack-depth finding), the wazmrt residuals (#17/#18/#23), and the open decisions |

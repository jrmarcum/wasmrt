...
## ◐ T13 CONFORMANCE CLEAR-OUT IN PROGRESS — **day 3 landed 2026-09-17**, unreleased
...
🎯 **Day 3 closed X1, X2, X3/T10b, the whole M/A track, and TRACK W.**
`wide-arithmetic.wast` went **0/1/108 → 107/0/0**; custom-page-sizes' 34 failures went to **0**; and
**`proposals/threads/` is at 497 / 0 / 0** after the owner-directed vendored patch (8 era-pinned
`assert_invalid` cases removed, each replaced by an annotated block quoting them — ⚠️ uncommitted in the
wasmtk tree on purpose, and designed to be overwritten by the next corpus sync, so **re-check then**).
**custom-descriptors is now 65/451 — 98% of everything that remains**; the rest is `custom/` 1/20
(track A) and custom-page-sizes 0/78 (track P, honest refusals). Detail in [roadmap.md](roadmap.md);
numbers in [testing.md](testing.md).

✅ **AND THE `Op`-SPACE QUESTION IS MEASURED, not open.** Widening `Op` to `u16` is **free on size**
(`Instr` stays 80 bytes — the second byte lands in padding `offset` already used), **cannot change
canonical behaviour** (`Op` is an internal tag; the 288-file suite under `u16` was **byte-identical**,
and `Op` never crosses the C ABI), and is **+7–9% FASTER** on the steady axis (A/B/A/B: u8 229.8 /
228.5 / 225.6 vs u16 250.1 / 243.4 Mops/s, groups non-overlapping). Cost: 13 `op as u8` sites.
⚠️⚠️ **The mechanism is unexplained — the number is measured, the cause is not** — so it is a **T11 lead
beside the unattributed ~5% regression**, not a headline. 🎁 Dividend: under `u16` the internal tags can
move to `0x100+`, out of the wire byte space entirely, retiring `decode_body`'s raw-internal-tag guard
and the whole §3A.2 class with it.

🔻 **THE CORPUS MOVED AND THE TOTALS HID IT.** wasmtk synced the vendored testsuite to upstream
`65a43d2e` on 2026-08-20 at 18:34 — after day 2's numbers were written. Day 3's true starting point,
re-measured rather than quoted, was **63,963 / 112 / 604 over 288 files**: +4 files, +156 passes, +20
skips. ⚠️⚠️ **The failure count was 112 on both sides by coincidence** (custom-descriptors +1,
custom-page-sizes −2, the new `custom/` dir +1), so the headline said "nothing changed" while four files
and 176 assertions had. 🎓 *When the corpus can move, the denominator is a measurement*
(`best-practices.md` §1.7). The new `custom/` directory also brought **work no track covered** — 20
skips of unhandled `assert_malformed_custom` / `assert_invalid_custom`, which is runner work with no
engine risk.

🔴 **TWO FINDINGS ABOUT GATES, and both are the same shape.** ⓵ `op_feature`'s doc comment claimed its
match was **"exhaustive over `Op`"** so a new opcode "shows up here as an explicit decision rather than
silently defaulting to always allowed". **It ends in `_ => return None`** — Track W's four ops landed
fully wired and **refusable by no flag at all** (measured: 182 ungated ops without the gate arm, 178
with). ⓶ Then `WideArithmetic` reached the Rust enum and the `Features` struct but **not**
`wasmrt_feature_t`, so it was switchable in-process and **not through the C ABI** — gated at one entry
point of two, which is exactly what X3 exists to prevent and what T10b had predicted. Both are pinned by
DATA now (`Op::ALL` + a count; a three-list comparison). 🎓 **A gate that cannot fail is decoration; a
gate that exists only in a doc comment is not even that** — and the comment asserting the protection is
the reason nobody looked (§4.4).

🔬 **A THIRD HOLE IN THE PER-FILE GATE.** The runner printed a row only for a file with failures or
skips, so a **clean** file had no recorded pass count and passes it later lost to **skips** were
invisible. X1 landed in it: `custom-page-sizes/memory_max.wast` went **2 passed → 0 passed / 6 skipped**
and `conformance-diff.ts` printed *"no file lost a pass"*. Every file prints a row now, and the same gate
then reported **four** affected files instead of two (§4.5).

⚠️⚠️ **TRACK D IS BLOCKED ON A REPRESENTATION DECISION, and it is not the one D1 names.** `Op` is
`#[repr(u8)]` and Track W's four tags took the **last four free bytes — zero remain**.
Custom-descriptors needs at least eight `*desc*` instructions and cannot be tagged this way at all: it
must either **widen `Op` to `u16`** (moving `Instr`, whose size is pinned, on the hot path — a T11
question landing inside T13) or **adopt the family pattern** `Simd`/`Atomic` already use (one tag, the
sub-opcode in the immediate). Route 2 looks right and costs nothing measurable, but it is D's call.

🔴 **AND ONE RECORDED DECISION TURNED OUT TO BE A NO-OP.** The era-pinned `proposals/threads/`
assertions were decided on 2026-08-20 as *"refresh the vendored snapshot"* — but the 08-20 sync **did not
touch `proposals/threads/` at all**, so the vendored snapshot already IS upstream's current state and
still carries the 6 stale assertions. There is nothing to refresh to. 🎓 *Verifying the premise of an
option is not verifying the option* (§8.4): "the snapshot is behind CORE" was proved and is true; "a
newer snapshot exists" was assumed. **Needs an owner decision again** — see `known-issues.md` for the
three surviving routes. Threads is nonetheless **13 → 8** with no snapshot change, and all 8 that remain
are assertions a modern engine must fail.

⚠️⚠️ **The day's finding is not in the conformance numbers at all.** Two of the fixes were **wire-format
divergences the spec suite structurally could not see**, because wasmrt's assembler and decoder agreed
with each other:

* every **non-null abstract reference type** was emitted as `ValType`'s *internal tag* — `(ref any)` as
  the single byte `0x66` — instead of §5.3.5's `0x64 <heaptype>`; and
* **`try_table`'s catch label was off by one**: assembler, validator and interpreter all agreed with
  each other, and all disagreed with the spec.

**wasmtime 47 refuses both.** 🎓 *Agreement between components that learned the convention from each
other is not evidence* (`best-practices.md` §3.8b). Handing wasmrt's output to an outside reader is one
command and found both in an afternoon; it is now how a format-level change gets checked.

⚠️ Four more of the day's fixes were **wrong ANSWERS wearing a conformance-failure label**: a flat-form
`offset=` was silently dropped (`i32.load offset=4` read at offset **0**), reference locals were
...
rather than shared. Each needed two modules or a shared store to be visible at all — which is why
63,000 assertions had not seen them.

🔬 **The instrument moved three times and paid each time.** Every assertion failure now names its
action; the decoder counts control nesting, so an unbalanced body is malformed at **decode** instead of
arriving as the validator's `ControlUnderflow`; and a skip reason that named the *wrong* cause was
found and fixed — `(get "x")` returned `NoTarget`, which renders as "an earlier module did not build",
so 11 skips were blamed on a module-build failure in two files where every module built
(`best-practices.md` §5.6b).

⚠️ The `.wat` corpus went **532/532 → 529/532**, and all three are **corpus defects** wasmtime refuses
on the same lines: two ArtOfWebAssembly files spell `anyfunc`, and `dynrt_lib_modc.wat` declares
`(local $alist i32)` four times in one function. Reported, not worked around — they are not our files.

🚦 **NEXT WORK (2026-09-17)** — `roadmap.md`, "DAY 3". The cross-cutting items and track W are
**done**; X1's prediction held exactly (skips rose 604 → 675 as silently-accepted modules became honest
refusals). What is queued, in order: ~~**track A**~~ *(✅ done 2026-09-19 — superseded wording follows)* — the `custom/` directory's two unhandled `assert_*`
commands, 20 skips of pure runner work; **the two owner decisions** (the threads snapshot, now that
refreshing is a no-op; and `Op`'s representation, which gates D); then **track P** (custom-page-sizes,
🔒 the memory-safety one — every bounds check is `pages × page_size`) and **track D**
(custom-descriptors, 65/451, largest and riskiest). ⚠️ The remaining prediction still stands:
**passes go DOWN when `exact` lands** — a parse gap currently scores as a correct rejection, so ~19 of
`exact.wast`'s passes are false.

🔴 **Scoping found a LIVE DEFECT, and it inverts one priority.** `(memory 1 (pagesize 1))`
**assembles, validates and RUNS today**, emitting bytes byte-identical to `(memory 1)` — the clause is
parsed and thrown away, so a guest asking for a 1-byte memory silently gets 64 KiB and every access
that must trap succeeds. wasmtime refuses the same **source**. ⚠️ The first cross-check against
wasmtime *agreed*, because it was handed our emitted bytes rather than the source — *when the emitter
is the suspect, compare the SOURCE across engines.* It is the **fifth** instance of the T10a emitter
...
### 2026-08-19 (T13 day 1) — the skip census, and the instrument that could not attribute its own number

Day 1 moved **62,238 / 378 / 2,038 → 63,333 / 172 / 1,024** across ~20 fixes: WasmGC array bulk ops,
table64 end to end, the assembler scoring split, §6.5.2 label repetition, typed `select`,
`br_on_non_null`, the relaxed-SIMD `either` form, `memory.init` reading its operands **backwards**, and
a dozen text-format / validator defects. ⚠️ *Adjudicated GREW, because skips became verdicts.*

🔬 **The day ended on the INSTRUMENT.** The runner listed a file only when it had failures, so **234 of
1,024 skips lived in files that printed nothing** — `ref_test.wast` (68), `ref_cast.wast` (42) and both
`br_on_cast*.wast` (27 each) had **never appeared in a conformance report** — and a skip was a bare
counter at all twenty sites. Both fixed; the census showed **928 of 1,024 skips (91%) were CASCADES**
behind a handful of roots, and only **96** were real gaps, resolving to seven items (S1–S7).
⚠️⚠️ Ranking by the skip column would have put a 108-skip cascade above the single root producing it.
🎓 *A number the report cannot attribute is not a measurement* (`best-practices.md` §5.6).
...
🆕 **THE LADDER WAS RE-CUT 2026-08-19 (owner). `1.0.0` is now a DELIVERABLE, not a summary.**

| stage | version | what |
| --- | --- | --- |
| **T13 — conformance clear-out** | **`1.0.0`** | the corpus to **zero failed / zero skipped / zero unrun**, empty baseline, **zero deliberate deviations** |
| T9 — hardening | `1.0.1` | the punch-list remainder + **`pin`** |
| T10 — bug hunt + code hygiene | `1.0.2` | the "look for code issues" trigger below, run as a scheduled task |
| T11 — optimization review | `1.0.3` | measured options, presented for a decision |
| T12 — security review | `1.0.4` | penetration surfaces, recommended plugs |

⚠️ **T-numbers are IDENTIFIERS, not an order** — they are cited across every file here, so the names
stay and only the ordering moves: **T13 → T9 → T10 → T11 → T12**, i.e. **clear → measure → find →
optimize → attack**. 🚦 T13 reverses two recorded scope invariants (*tables stay 32-bit*; *untargeted
proposals: record, do not fix*) — that is the owner's decision, recorded in `roadmap.md` and
`design-decisions.md` rather than drifted into. **T13-0 — instrument the skips and audit the runner's
SCORING — runs before any implementation**; over in wazmrt that phase alone turned 292 skips into passes
and reclassified 14 of 104 failures, with no proposal code written. **Rank by ASSERTIONS UNBLOCKED, not
by failure count**, and expect the failure count to RISE honestly as skips convert — the gate is *no file
lost a pass*, per-file, never the totals.

*(Superseded: "Next: T9 (0.10.0)… then T10 (0.11.0), T11 (0.12.0), T12 (0.13.0)". T9's scoping detail is
in `roadmap.md`; tail calls landed 2026-08-14.)* **T11 and T12 both end in a
...
2. ⚠️⚠️ **AUDIT for stale LIVE claims — do not just edit the files you happened to touch** *(added
   2026-08-19)*. The edits you make go to the paragraphs the work reminded you of; **the headline is the
   line nobody greps for, and it is the one most likely to be quoted.** The audit that works:
   **grep for the OLD VALUES** (`grep -rn "62,113\|533/533\|457 tests" cmem/ README.md CLAUDE.md`), then
   classify every hit as **live** (fix) or **dated history** (leave, and mark it superseded if it claims
   to be current). ⚠️ **When two files disagree, do not pick the newer one — MEASURE.** Watch for the
   words *"final"* and *"the number to quote"*: both have appeared on figures superseded within days.
   Re-read the **"by design" / "out of scope" / scope-invariant** lines whenever a track closes — nothing
   about them fails a build, so they are the entries least likely to be checked and most likely to read
   as current; **a correction is a dated claim too.** *(Borrowed method, `best-practices.md` §3A.2; the
   first run of it on 2026-08-19 found the roadmap's own status block five days stale and every recorded
   `.wat`-corpus figure quoting a denominator that no longer exists.)*
3. **Sync `README.md` only where the change is user-relevant** (status, eventual install/usage/API).
...
| File | What it holds |
| --- | --- |
| [interop.md](interop.md) | 📎 **2026-09-20: wasmrt ANNEX §2.2f (unnumbered, regime A) — a flag row verified on PARSING is not verified.** `--features` was ✅ DONE/AGREED on vocabularies, seeding, layering and position, and was applied by **one of the three wasmrt paths that judge a module**: `wasmrt run --features mvp simd.wasm f` executed it. Measured on both runtimes; **wazmrt applies it on both of its paths**, so the break was ours. Fixed, pinned by `cli_features_are_applied.rs`, no contract change proposed — the rule is a VERIFICATION rule: assert on what the command DID, on every path the flag governs. · 🔒 **2026-09-19: THE PIN DB PATH IS DECIDED (owner) — the shared `/etc/wasmtk/pins` + `C:\ProgramData\wasmtk\pins`, per-runtime fallback, and a LOUD warning when only the sibling's path has a DB, so a swap cannot silently disarm verification. §3.7a (the execution bound) and §5 decisions #1/#5 are CLOSED; §2.1/§2.2's wasmrt column is now ✅ across the board. 🆕 §3.1m records that `.wat` pin digests DIVERGE (Z4).** Also 🔒 **2026-09-19: OWNER DECISIONS §2.4a (unknown flags → `unknown flag`, rc 1, by position) and §2.5 (a validity claim must match the verdict; the one output-text exception to §0); a §2.3 Z1 row; §2.5h hands Z1–Z3 to wazmrt. All unnumbered, awaiting the pen-holder's fold-in.** Also 📎 **2026-09-19: wasmrt ANNEX §2.3m (unnumbered, regime A)** — §2.3 measured by running both; wasmrt fixed three breaches of row 2; row-3 promotion proposed; Z1–Z3 for wazmrt, **Z2 (unknown flags) needs the owner**. The copies differ until wazmrt adopts it. Earlier: 🤝 🆕 **THE wasmrt ⇄ wazmrt SWAPPABILITY CONTRACT (owner, 2026-08-19) — now CONTRACT VERSION 2.** 🔒 **v2 settles §2: the two EXECUTABLES keep their own names and that is the ONLY permitted difference — everything after the program name must be in sync**, so both adopt both spellings and there is nothing left to decide there. The working test: *change only the program name and the invocation must do the same thing.* ✅ **§3.7a added — the ITERATION BUDGET, decided in wazmrt (Track H3) and adopted verbatim on the owner’s instruction**: a COUNT not a clock, default `1 << 30`, refilled per top-level invocation, `0` = unlimited at the CLI but = default in the C ABI, and deliberately **not** admitted as a runtime trap by the `.wast` runner. ⚠⚠ **It ticks in TWO places and the second is the one that matters — the TAIL-CALL TRAMPOLINE**, because a local `return_call` reuses the native frame, so it makes no backward branch and grows no call depth: `(func $f (return_call $f))` runs forever in wasmrt **today**. Work item = **T9i**. 🚦 **§3.3 the pin DB PATH is ESCALATED** — the owner wants it discussed with the other project for security’s sake, and **until it is settled neither side may ship an armed gate**, since separate paths make a swap compute `armed = false` and **silently run everything**. 🚦 §2.3 exit codes need coordination. Previously (v1): CONTRACT VERSION 1, and this file is IDENTICAL in both repos.** The **only** document either project may treat as binding on the other; 🔒 the oracle stays retired, this is a contract about **observable behaviour**, not a licence to follow a competitor’s design. **IN scope: the CLI options + the security checks. OUT: the C ABI** (the `wasmrt_*`/`wazmrt_*` prefixes stay distinct by design), internals, and **performance/size — which is what they are competing on.** Carries the run-mode and flag diff, the nine-row `decide()` matrix, the pin-DB format, the fail-closed rules, the WASI rights rows and the ceiling defaults (✅ memory `1<<30`, table `1<<27`, call depth `512` — verified equal in both). ⚠⚠ **Two breaks are LIVE today**: `--dir` uses `:` vs `::`, so `--dir .:/` silently preopens the wrong thing here; and aligning run modes makes a **bare path EXECUTE** a `_start` module where wasmrt summarizes. ⚠⚠ **The unresolved row that matters most is the pin DB PATH** — separate paths mean a swap finds no DB, computes `armed = false`, and **silently runs everything**. ⚠ **Named drift hazard: the file exists TWICE** (*a list written out a second time will drift*), so the pin is the CONTRACT VERSION plus “neither side edits unilaterally”, and check #1 of §4 is a byte-compare of the two copies. Also carries the 5 differential checks and the 4 owner decisions it is waiting on. |
| [best-practices.md](best-practices.md) | 🆕 **§1.10 a grammar production has more than one SITE and the failing test names one (the element-segment reftype was fixed alone; the table-type one ran for the whole project) · §1.11 SWEEP THE AXIS, NOT THE ARM (and a sweep's control arm is what stops a broken probe certifying a gate) · §1.12 a differential fuzz reaches what a TEXT-format suite structurally cannot · §3.14 a field recorded for ONE consumer is not a rule the others follow (`Table::is64` was read only by import matching) · §4.11 a contract row verified on PARSING is not verified (2026-09-20)** · 🆕 **§1.9 a SECOND AUTHORITY on a grammar fails on whatever was added after it was written — and a probe that picks the BENIGN input reports "no defect" in the same words as a correct implementation · §3.13 READ-AND-DISCARD IS NOT VALIDATION (five fields whose undefined values were accepted; *a ceiling is not membership*) · §3.8d the OUTSIDE READER is a gate now, not a good habit · §8.1b line endings are NOT uniform within this repo, so a scripted edit must assert its own application (2026-09-19)** · 🆕 **§4.1b a guard COMPUTED and never CONSULTED is not a guard (the compiler will not warn: a `pub` field of a `pub` struct is not dead code) · §5.4f a wall-clock assertion in a parallel runner measures the MACHINE.** Earlier: 🆕 **§4.10 parsing a flag proves it was ACCEPTED, not APPLIED (test the spellings, not the flag) · §5.4e a fixture must be able to take the path it is testing.** Earlier: 🆕 **2026-09-19 — TEN new entries from one session: §1.1b a comment outlives what it describes (three found, each contradicted by code a few lines away) · §1.4a fixing a lying exit status beats a rule that works around it · §1.8 enumerate a small input space instead of reading the table · §2.5 running answers BEHAVIOUR, not PROVENANCE · §4.2b a gate you PORT must be re-proved able to fail · §4.9 one ceiling over two paths needs a check on each · §5.4a an invalid-module test must be invalid for exactly one reason · §5.4b two implementations agreeing is a hypothesis until one command compares them · §5.4c a test needing a specific failure must change the OUTCOME · §5.4d a feature flag is part of a test INPUT.** §2.5: running the artifact answers BEHAVIOUR, not PROVENANCE — a binary's `process.versions` and its string table describe the TOOLCHAIN, not the language it is written in; for what-is-it-built-in / who-owns-it / when-did-it-change, cite the project's own release notes. Earlier: 🆕 **2026-09-19: §1.8 enumerate a small input space instead of reading the table; §5.4a an invalid-module test must be invalid for exactly one reason.** Earlier: 🆕 **§4.1a (2026-09-19): a comparison gate's "agree" must count only what it COMPARED** — two malformed probes, refused by both tools, read as "7/7 agree". Before that: 🆕 **FIVE NEW SECTIONS, 2026-09-17.** **§4.4 — a property asserted in a DOC COMMENT is not a check**: `op_feature` claimed to be "exhaustive over `Op`" and ends in `_ => return None`, so Track W's four ops shipped **unrefusable**; the comment is worse than nothing because it is the reason nobody looks. **§3.12 — an unknown spelling is not automatically refused, it can be RE-READ** as something else (`(elem 0 …)` became a passive segment), so test the MEANING, not acceptance. **§4.5 — a clean file has no row, so it cannot lose a pass** (and a shell probe you build for yourself is still a gate: check it can fail). **§1.7 — when the corpus can move, the denominator is a measurement**, and an unchanged total is not evidence. **§8.4 — verifying the premise of an OPTION is not verifying the option**: "the snapshot is behind core" was proved; "a newer snapshot exists" was assumed. Earlier: 🆕 **THREE NEW SECTIONS, 2026-08-20.** **§3.8b — three components that agree are not three witnesses**: wasmrt's assembler emitted its own internal `ValType` tags into the binary and its decoder read them back, so every round trip was green while wasmtime refused the output — *only a reader that did not write the bytes can tell a convention from a bug*. **§5.6b — a reason that names the WRONG cause is worse than no reason**: `(get "x")` returned `NoTarget`, which renders as "an earlier module did not build", so 11 skips were blamed on a module-build failure in two files where every module built. **§5.7 — a leniency with no wrong-value consequence is still a deviation**: `anyfunc`, the three hierarchy bottoms and legacy `delegate` all survived because they could not produce a wrong answer, and a rationale ("matching the oracle") outlived the oracle by nine days in three separate comments. Then: **§5.5a — read the skip column to RANK, work the FAILURE column to FIX (2026-08-19).** The inverse of §5.6, and both are true because they answer different questions: a failure count undercounts a skip-symptom defect (so rank by skips), but a failure is a root you can act on while a cascade is not actionable until its root moves (so fix by failures). ⚠️ The residue after a failures-first pass is *by construction* the skips no failure could reach — which is where the biggest item was hiding. 🆕 **§5.6 — a number the report cannot ATTRIBUTE is not a measurement (2026-08-19).** "1,024 skipped" was printed in every release summary for the whole port and was unscopeable: the report filtered on `failed > 0` and the skips carried no reason. **A count with no cause cannot be ranked** — planning off the skip column would have put a 108-skip cascade above the single root producing it. **Ask what the report OMITS, not only what it says** (the sibling of §3.8), and **instrument before triaging, every time**. 🆕 **§3A.2 — SECOND BORROW ROUND (2026-08-19): wazmrt's method since 08-14, filed per wasmrt task.** For **T11**: ⚠️⚠️ *a benchmark whose FLOOR is larger than its signal measures the floor* (a ~30 ms spawn floor hid a **20–55×** engine difference and flattened a real size dependence into "nothing moves"); quote the DIFFERENCE, not a load-dependent ratio; ⚠️⚠️ *a size number must be reproducible in the CONFIGURATION it was recorded in — including WHERE* (one unchanged commit measured **four** static-archive sizes, varying only with the source-tree path — **and the never-measured rlib IS that kind of archive**); bisect the ~5% regression against parents rather than reasoning about causes; **a FIX task and a COMPARE task do not belong in one queue.** For **T10a**: 🚨🚨 *a ROUND TRIP PROVES AGREEMENT WITH YOURSELF* — wazmrt emitted an internal tag raw for **nine months**, invalid to every other runtime, its corpus byte-identical before and after the fix, **so T10a's planned gate could not have caught it**; add an external arm. For **triage**: *instrument a skip, do not reason about it*; *a skip total is dominated by CASCADES*; *a "0 failed / 0 skipped" corpus is not "everything ran" — read the file-error count*; **a refusal is not a hole.** For **T12**: *an enforcement arm running BESIDE what it constrains enforces only what its caller remembered to ask for*. For **tests**: ⚠️ *an inversion that catches NOTHING has three causes and only one means delete*; 🔒 *for a soundness rule, write the WRONG ANSWER down*. For **memory**: ⚠️⚠️ *"update the project memory" means AUDIT for stale live claims* — grep the OLD VALUES, and **when two files disagree, MEASURE**. For **this machine**: ⚠️⚠️ *finding a real defect at a layer is not evidence it causes your symptom* — the `D:` blocker was misdiagnosed **four times** (a stale event log, antivirus, and a genuinely damaged volume that was repaired and changed nothing); the cause is the **cache LOCATION**. Plus §3A, the first borrow round (2026-08-14). Then: 🎓 **The lessons this port paid for, each with the defect that bought it** (2026-08-08). **Read it before starting a task.** Measurement (**a logged cost is a hypothesis about a CAUSE — wrong 4×**; ⚠️ **a SCOPE note is one too, and nobody re-measures it** — "annotations 51, a proposal we do not target" was true of the file and false of 44 of its 51 assertions, which were generic lexer rules;; survey the measurement, not the entry; know which direction your tooling can lie in; same-session A/B/A; ⚠️ **the PLUMBING for a cold feature can cost the hot path** — trap backtraces are diagnostics, but getting `pc` out of `run` the obvious way measured 3.6% slower, so benchmark anything touching a hot loop's locals, and pin any size/layout claim with a `size_of` test). What to distrust (**a stated constraint is worth PROBING, not agreeing with**; a design doc is a hypothesis; an enum's numeric order is not the wire order). Where the defects are (**silent-wrong-output is the worst class and where every serious defect has been**; ⚠️ **a feature can be fully decoded, validated and PRINTED and still never execute** — the start function never ran for five releases while `start.wast` failed, so check that the feature a file is NAMED for works before diagnosing its messages; **the emitter reconstructs a form from a SUBSET of the parser's facts** — 3×; **comparing STRUCTURE cannot answer a question about IDENTITY**; an asymmetry between layers is a bug; the two-instance rule; **rejection STAGE is part of correctness**). Checks and gates (**a gate that cannot fail is decoration**; mutation-verify; **a guard one call-level away from the iteration it guards is NO check**; **a blanket skip insures the code it covers**; **which direction to err in is a property of the CONSEQUENCE, not a house style**; loud `Unsupported` over silent-wrong; a library must never abort the embedder). Tests (**assert where a check STOPS working**; every test asserted the module *runs*, so defects that ran were invisible; our own fixtures were malformed). Untrusted input (O(n log n) not O(n²); decode must be total; say what you did not do). The 🔒 **wasmtime shape-not-code** rule and its compliance test. Working method. |
| [overview.md](overview.md) | 🆕 **T13 day 1 (2026-08-19): 63,333 / 172 / 1,024 — 99.7% of 63,505, 484 tests**, +1,095 passes and −206 failures in one day, no file lost a pass. What wasmrt is, **why the oracle was retired (2026-08-11)**, repo/crate layout, mental model, and the **stage-by-stage status through T9's eighteen passes** — **62,238 / 378 / 2,038 (99.4% of 62,616), 458 tests**, ✅ tail calls landed so **no in-scope proposal is missing**, ⚠️ the `.wat`-corpus figure UNVERIFIED (its denominator moved), and the **defects no list had**, not one of which was found by reading the punch-list. *(Re-audited 2026-08-19 — this row and the file both still said "eleven passes, 99.3%, 426 tests".)* |
| [vision.md](vision.md) | 📌 **2026-08-19 — the competitor published head-to-head numbers FIRST**: C-ABI shared library **wazmrt 222 KB vs wasmrt 554 KB** (it calls the embed footprint its strongest card), CLI **890 KB vs 684 KB** (wasmrt smaller), and its bake-off notes **wasmrt ties it end to end**. ⚠️ Not adoptable — not feature-parity-verified, and those figures disagree with our own **493.5 KiB / 621 KiB**, because build config and date are part of a size number. **It raises the priority of the missing same-box comparison, it does not close it.** The one thing worth keeping without re-deriving is the shape: **the cdylib is where wasmrt is behind, and the cdylib is what every C consumer links.** Then: 🔒 **WHAT wasmrt IS COMPETING FOR (2026-08-11): inclusion in wasmtk + universalWasmLoader-*, decided on the smallest and fastest binary; rsxtk takes it by default via the native Rust interface. TWO ARTIFACTS with different levers — the rlib rsxtk links (never measured) and the cdylib the C consumers link (493.5 KiB). FAST AND SMALL ARE NOW THE GATE, not aspirations with a footnote.** The goal — **plus where the three axes actually stand, now that ALL THREE are measured (2026-08-07)**: *canonical* 98.9%; *fast* **4.48 ms cold at 48 KB / ~237 Mops/s steady**; *small* **CLI 621 KiB, cdylib 493.5 KiB, freestanding wasm32 engine 158.1 KiB → 137.5 KiB with `wasm-opt -Oz`**. What is still missing on fast/small is the **comparison to another runtime** (wasm3/WAMR/Deno), which needs those binaries present. Still: do not quote wazmrt's benchmarks as wasmrt's. fast + smallest-binary + wasm-compilable runtime that **replaces wasmtime** under the `universalWasmLoader-*` projects |
...
| [testing.md](testing.md) | 🆕 **2026-09-20 (validator review): three new differential gates — `scripts/feature-gate-sweep.ts` (18 proposals, each disabled, both engines must refuse), `scripts/index-type-sweep.ts` (23 instructions × 2 spellings, then 10 executed past `2^32` against wasmtime) and `scripts/differential-fuzz.ts` (`wasm-tools smith` + one-byte mutations). Each was proved to FAIL by reverting the defect it found. **588 workspace tests, `tests/*.wast` 230 / 0 / 0 over 17 files, conformance unchanged at 64,603 / 0 / 0.** ⚠️ Set the outside reader's feature list to OURS: `-f all` enables proposals wasmrt does not implement, and reported seven false 'defects' before it was narrowed.** · 🆕 **2026-09-19 (day 5): `scripts/wat-corpus.ts` — the `.wat` gate ends at an OUTSIDE READER (assemble → decode+validate → `wasm-tools validate`), reading **528 / 532 files with all 528 accepted by wasm-tools**; `tests/decoder-strictness.wast` joins the regression corpus; Miri's recorded count corrected to 32. ⚠️ A bare `(module binary …)` directive is scored only when it FAILS, so a file's total is outcome-dependent.** · 🔒 **2026-09-19 (part 13): `cli_wasi_hardening.rs` — the sandbox escape and three allocation DoS cases, guest-driven; ⚠️ the escape test needs a real FILE symlink, and a directory junction is NOT a substitute (it reports a false all-clear).** Earlier: 🔧 **2026-09-19 (part 11): the four gates are TypeScript, run by Deno or Bun, and each was verified to still FAIL when it should.** Earlier: 🔒 **2026-09-19 (part 10): 564 tests; the pin gate, the run modes and the iteration budget each pinned; the ceiling re-measured on our own corpus; SHA-256 cross-checked against `sha256sum`.** Earlier: 🔒 **2026-09-19 (part 9): 535 tests; `cli_unknown_flag.rs` pins §2.4a.** Earlier: 🤝 **2026-09-19 (part 8): 529 tests; `wasmrt wast` and `wasmrt <file>` exit statuses are now usable gates.** Earlier: 🆕 **2026-09-19 (part 7): `tests/value-type-encodings.wast` — wasmtime 48 passes it whole; `tests/` 13 files / 199 assertions; the one-byte-sweep method.** Earlier: 🎯 **2026-09-19 (D3/D4) — 64,598 / 0 / 0: ZERO failed / skipped / unrun; the runner picks FEATURES PER FILE (`wast::features_for_script`); `assert_trap` does not compare messages, so specific-trap tests must change the OUTCOME; `.wat` corpus 531/535, custom-sections 531/4/0.** Earlier: 🆕 **2026-09-19 — 64,087 / 65 / 530; new EXTERNAL gate `scripts/custom-sections-diff.ts`** (wasmrt vs wasm-tools, section order + custom-section bytes; baseline 528 agree / 2 refused by both / 2 differ); C-ABI gate needs `CC=gcc` on this host. Before that: 🆕 **2026-09-17 (T13 day 3) — 64,068 / 66 / 549, 99.9% of 64,134, over 288 files**, and **`proposals/threads/` is at 497/0/0** after the owner-directed vendored patch; 503 workspace tests; C-ABI gate PASSED; `.wat` corpus **528/532** ⚠️ *(not 530 — the gate keyed on an exit status that is always 0; two pre-existing failures were invisible. All four are corpus defects wasmtime also refuses.)* ✅ **Miri 31/31 PASSED** (31, not the 28 on record — the crate gained tests). 🔻 **THE CORPUS MOVED AND THE TOTALS HID IT**: wasmtk synced to upstream `65a43d2e` on 08-20 at 18:34, so day 3's real starting point was 63,963 / 112 / 604 over **288** files, not 63,807 / 112 / 584 over 284 — and **the failure count matched on both sides by COINCIDENCE**. 🔬 **A third hole in the per-file gate**: a clean file had no row, so passes lost to SKIPS were invisible — `memory_max.wast` went 2 passed → 0 and the gate said "no file lost a pass"; every file prints a row now and the same gate then found four. 🔬 **X2 shipped as CLAUSE coverage, not field coverage** (39 rows, mutation-tested at three emitter sites) because a field can only be covered once it exists. Earlier: 🆕 **2026-08-20 (T13 day 2) — 63,807 / 112 / 584, 99.8%, and the 257 CORE files are at 0 failed / 0 skipped.** All that remains is inside `proposals/`. ⚠️⚠️ **`wasmtime compile out.wasm` is now part of checking a format-level change**: two of the day's fixes were wire divergences the spec suite structurally could not see, because wasmrt's assembler and decoder agreed with each other. Then: **2026-08-19 — THE SKIP CENSUS.** The runner listed a file only when `failed > 0`, so **234 of 1,024 skips lived in files that printed nothing** — `ref_test.wast` (68 skips), `ref_cast.wast` (42) and both `br_on_cast*` (27 each) had **never appeared in a conformance report**; and a skip was a bare counter at all twenty sites, so even a listed file could not say why. Both fixed (`Summary::skips`, a per-file listing, a corpus-wide census), pinned by `every_skip_records_a_reason`, mutation-verified with the mutation's arrival confirmed first. **928 of 1,024 (91%) are CASCADES**; only **96** are real gaps. 🎓 *A number the report cannot attribute is not a measurement.* Conformance table extended: **63,333 / 172 / 1,024 — 99.7% of 63,505**, and ⚠️ the denominator MOVES, so never read the rate alone. ⚠️ **2026-08-19 — the `.wat` corpus figure is UNVERIFIED and must be re-run before it is quoted.** The corpus is **not ours** (it lives in the `wasmtk` tree, which is live), and it holds **532** `.wat` files today; every recorded figure — 533/534, 534/534, 533/533 — is against a denominator that no longer exists, the last honest measurement being T9a#8's *534/534 assemble, 0 decode failures, 2 validate failures*. Those 2 were the `39_JstyperMixed` pair **T9a#9 adjudicated as an invalid stale fixture**, and they are now absent from wasmtk — which accounts for 534 → 532 exactly, making *532/532* the likely truth and **still an inference, not a measurement**. 🎓 Three of our own rules converge here: a number must be reproducible **in the configuration it was recorded in** (a corpus you do not own is part of that configuration); **when two files disagree, measure**; and **quote all four numbers** — passed / failed / skipped **and the file-error count**, since a file that dies before it runs appears in none of the first three. Also 🆕 **`tests/*.wast` now runs under `cargo test`** (`regression_wast.rs`, zero failures AND zero skips, fails if empty) — the three reproducers were green and NOTHING RAN THEM. ⚠️⚠️ **`tail_call_is_a_real_tail_call.wast` is the test the spec suite cannot replace**: with the fake `return_call_ref` restored by mutation it fails 5/6 while the tail-call spec files still read 38/6, 72/4, 40/6. 🔒 **THE ORACLE IS RETIRED (2026-08-11) — conformance now anchors on the spec testsuite, wasmtime's observable behaviour, the wasmtk WASI corpus and wasmrt's own 456 tests, in that order of authority; two T9 punch-list items were FALSE LEADS produced by the old parity arrangement.** Previously the parity/oracle split — **live conformance 99.4% (62,113 / 385 / 2,163 of 62,498) and 438 tests as of 2026-08-08; the `.wat` corpus assembles 534/534 with 0 decode failures — and why that gate must run assemble→decode→validate, not `wat -o` alone**; the decoder-strictness column (+98 passes, −98 failures, **skips unchanged**: no accounting movement, 98 assertions simply started passing) and why a test that pins where a check STOPS working earns its place; the 08-08 column (+346 passes / −463 skips, **no file losing a pass**) and why **a blanket skip is not a neutral placeholder** — `assert_unlinkable` was skipped for a reason obsolete since T7b and was insuring a real engine defect; the runner now separates *"nothing defines this import"* (a real unlinkable verdict) from *"wasmrt cannot back this kind"* (a gap → skip), which is exactly what had made the assertion unimplementable; the 08-07 column split into *real capability* vs *honest re-accounting* (the runner's failed-build redirect was inflating failures); the **T9c benchmark** and why it asserts its own result before timing; the methodological finding that **a cost logged beside a defect is a hypothesis about its cause**; **the three C-ABI gates and what each catches that the others cannot** (link completeness / header-is-valid-C / Miri sees a UAF a normal allocator cannot); the mutation check on handle tagging; **351 tests**; Rust↔wazmrt golden vectors for shared features; **the vendored spec testsuite + its live conformance numbers (98.8% — 61,033/738/3,075 at v0.9.0; all 284 files parse) and where it lives**; **why the total adjudicated count moves when modules start building** (a module that fails to build costs exactly ONE skip, not one per assertion); `wasmrt-capi` under Miri + a lifecycle fuzz; a wasi-gate compiling real guests; bench cold vs steady; DoD = full parity on both targets |
| [security-model.md](security-model.md) | 🔴 **2026-09-19: the sandbox had an ESCAPE — an unfollowed final symlink was opened by the OS; fixed, and the T12 surface now has its first dedicated review (`known-issues.md`, top).** Earlier: 🔒 **2026-09-19: T9e IS BUILT** — `pin.rs` (our SHA-256, NIST + `sha256sum` verified), load-once as a TYPE, the gate before validation on every executing path including `wast`, `wasmrt pin`, and the owner's shared-DB-path decision. ⚠️ **`.wat` digests are not portable between the runtimes (Z4).** Earlier: ✅ **DECIDED 2026-08-19 — gate load-once, build `pin` to wazmrt’s APPLICATION, and be SWAPPABLE.** The owner scoped swappability as **the CLI options + the security checks** (not the C ABI — the `wasmrt_*`/`wazmrt_*` prefixes stay distinct by design), so the file now carries a **buildable spec**: the pure `pin.rs` core (`Mode Off<Warn<Enforce`, `stricter`, the pure `decide(explicit, pinned, opt_out, tty, armed)`, the content-addressed plaintext `Db`) and the CLI wiring (gate on `will_execute`, **before** validation; root DB authoritative; `--verify` may only RAISE). ⚠️ **Two fail-closed rules are load-bearing**: an unrecognised `# mode:` value means **Enforce**, not “no policy”; a non-hex DB line is an **error**, not a skipped line. ⚠️ **wasmrt must implement SHA-256 itself** (Rust `std` has none; zero-deps is an invariant) — fine here because there is no key and no secret, pinned by NIST vectors, and the argument explicitly does **not** extend to Ed25519. ⚠⚠ **`wasmrt wast` must be gated** — that exact bypass shipped over there: a `.wast` invokes the modules it contains, any wasm can be wrapped in one, and the attacker picks the extension. ⚠⚠ **`--dir` separators already differ** (`:` vs `::`) so `--dir .:/` silently preopens the wrong thing on wasmrt — a swappability break live TODAY, unrelated to pin. ⚠⚠ **Aligning run modes makes a bare path EXECUTE** a `_start` module where it now summarizes — land the gate first. ✅ **Load-once already holds** (`read_module_bytes`, one `fs::read`) and becomes a **type**, not a comment: `Loaded { bytes, digest }`. Also — the REVIEW behind all of it (the premise did not hold: wazmrt does BOTH, and load-once is what makes its gate TOCTOU-safe). The premise did not hold: **wazmrt does BOTH** (`pin.zig` SHA-256 allow-list + `sign.zig` Ed25519, armed by default when a root key or DB is present), and load-once is how its gate is made TOCTOU-safe — which is what this file already said (*hash the in-memory bytes about to run, never re-read by path*). ✅ **wasmrt already HAS load-once**: every executing path goes through `read_module_bytes` (`crates/wasmrt/src/main.rs:200`) — one `fs::read`, owned bytes to execution, and the C ABI never opens a file. ⚠️ **But it is a doc comment, not a gate**, so an `mmap` at T11 could remove it silently — that test is the missing part. 🚦 **The two answer different questions**: load-once = integrity over time (mid-run swap); pin = authenticity (the file replaced BEFORE load, which needs no race and is the easier attack). **Recommendation: gate load-once, re-scope `pin` to default-`off` rather than delete it** — and if it is dropped, say so in `wasmrt.h` and the README, because an undocumented gap gets found twice. Also 🔒 **THIS FILE IS NOW AUTHORITATIVE (2026-08-11)** — it used to defer to wazmrt's copy; that pointer is retired with the oracle, and the file itself records WHY inheriting was wrong: wazmrt had **no `path_symlink` right at all** while wasmrt gated creation correctly, so faithful replication would have replicated the hole. ⚠️ **`pin` is a STUB — Authenticity is NOT implemented.** 🔒 **Symlink CREATION denied by default (owner, 2026-08-10)** — `--dir` grants `ALL & !PATH_SYMLINK`, `--allow-symlink` opts in for installer work, `--ro-dir` denied it already; enforced by `const` assertions so a violation fails the BUILD, and mutation-verified. ⚠️ **Governs CREATION, not traversal** (following a pre-existing link needs `PATH_OPEN`, which both grants keep — also asserted). Composing modules over shared memory is the STORE's job, so a workload run never needs new links; denying creation shrinks what an external racer can repoint, which is what makes the accepted TOCTOU residual survivable. ⚠️ The oracle had **no `path_symlink` right at all** and wasmrt did not — see roadmap **T12x**. Also 🔒 **Validation is a SECURITY boundary and every entry point must cross it (2026-08-10)** — `wasmrt run` did not, so an ill-typed module executed and printed a plausible answer while `wasmrt wasi` refused the same bytes; the oracle was worse (**both** execute paths + `wasm_module_new`). Carries the entry-point table, why the Rust `Instance::new` split is the deliberate exception, and ⚠️ **the same missing check is a different SEVERITY in the two languages** — `forbid(unsafe_code)` bounds wasmrt to a wrong answer, while Zig ReleaseFast reached a segfault from a 31-byte module. Plus: 🔎 **A dedicated adversarial review is scheduled — T12 (0.13.0)**, framed on three adversaries (hostile guest / malformed input / misusing embedder); two surfaces already listed from the release profile alone: `panic = "abort"` makes any reachable panic a **host-process kill**, and no `overflow-checks` in release means arithmetic **wraps there and panics in debug**. ⚠️ **`pin` is a STUB in wasmrt — a build today performs NO authenticity check**; that section describes the ORACLE, not current behaviour (T9). The *authority* half (the WASI sandbox) IS built. **Two new authority controls at T8:** proposal gating + resource ceilings, both reachable from C. Sandbox **secure by construction** + ⚠️ the **CORRECTED resolver note (2026-08-05)**: Rust's `std` has **no** dir-relative open, so zero-dep + no-`unsafe` + hold-real-handles cannot all hold. Shipped a component-accumulated path — every **escape** property intact, **inode pinning** lost, `verify_beneath` compensating; closing the residual was **DECIDED 2026-08-05: accept + document** (zero-dep and zero-`unsafe` both held). Also: rights-narrowing preopens; the mandated canary test (mutation-verified); pin verify (hash the in-memory bytes you run; root-owned DB; enforce denies before opt-out); authenticity vs authority |
...
| [roadmap.md](roadmap.md) | 🔴 **2026-09-19 (DAY 5): the DECODER REVIEW — 8 defects + a 9th in the assembler; the handoff STATE line is refreshed (64,603 / 0 / 0, 580 tests, `.wat` 528/532 all accepted by wasm-tools).** Earlier: 🔒 **2026-09-19 (DAY 4 part 13): the WASI review — 10 findings, 580 tests.** Earlier: 🔒 **2026-09-19 (DAY 4 part 10): T9e (the pin gate, load-once as a type, `wasmrt pin`) and T9i (the execution bound) are DONE, with the CLI converged on wazmrt's run modes and flags. `.wat` pin digests diverge → Z4.** Earlier: 🔒 **2026-09-19 (DAY 4 part 9): owner decisions §2.4a (unknown flag) and §2.5 (Z3) recorded; Z1–Z3 handed to wazmrt through the contract; wasmrt complies.** Earlier: 🤝 **2026-09-19 (DAY 4 part 8) — `coordinate` on exit codes: three wasmrt breaches of an AGREED row fixed; annex §2.3m offered to wazmrt (Z2 needs the owner).** Earlier: ✅ **2026-09-19 (DAY 4 part 7) — handoff item 7 DONE (the value-type decoder: three defects, one reader); next is `1.0.0` release prep.** Earlier: 🎯 **2026-09-19 (DAY 4 part 6) — TRACK D DONE, corpus at 64,598 / 0 / 0; HANDOFF now: wasmtk commits the threads patch → the two OPEN decoder defects → 1.0.0 release prep.** Earlier: 🆕 **2026-09-19 (T13 DAY 4) — 64,087 / 65 / 530; TRACK A DONE as the custom-annotations feature** (owner: keep the information, align with canonical). `@custom`/`@name`/`$id`s/branch hints now emitted byte-identical to wasm-tools (528/532 of the `.wat` corpus via the new external gate); malformed → refused, non-branch hint → emitted + reported. Earlier the same day: the runner's whole-module-quote wrapper was **manufacturing 6 false passes**, hiding a silent duplicate-`start` wrong module and misplaced legacy handlers. 📏 **Cost: CLI +51.7 KB, cdylib +59.9 KB — and a T11 LEAD: the cdylib already carries the assembler AND the `.wast` runner, which no C-ABI path reaches.** Next: the import type-use check. Before that: 🆕 **T13 DAY 3 LANDED 2026-09-17 — X1, X2, X3/T10b, the whole M/A TRACK and TRACK W are DONE.** **64,068 / 66 / 549 over 288 files**; `wide-arithmetic.wast` 0/1/108 → **107/0/0**; custom-page-sizes 34 failures → **0**; **`proposals/threads/` 497/0/0** after the owner-directed vendored patch (8 era-pinned `assert_invalid` cases removed, each replaced by an annotated block quoting them — ⚠️ uncommitted in the wasmtk tree on purpose, and designed to be overwritten by the next sync, so **re-check then**). 🚦 **custom-descriptors is now 98% of everything left** (65f/451s). ✅ **The `Op`-SPACE QUESTION IS MEASURED**: widening `Op` to `u16` is **free on size** (`Instr` stays 80 bytes), **cannot change canonical behaviour** (internal tag only — the 288-file suite under `u16` was **byte-identical**, and `Op` never crosses the C ABI), and is **+7–9% FASTER** on the steady axis (A/B/A/B, non-overlapping); cost is 13 `op as u8` sites. ⚠️⚠️ **Mechanism unexplained — the number is measured, the cause is not**, so it is a **T11 lead beside the unattributed ~5% regression**, not a headline. 🎁 Dividend: tags move to `0x100+`, out of the wire byte space, retiring `decode_body`'s raw-internal-tag guard and the §3A.2 class. 🆕 **A new track A** — the `custom/` directory arrived with the 08-20 corpus sync and no track covered it: 20 skips of unhandled `assert_malformed_custom` / `assert_invalid_custom`, runner work with no engine risk. Queue: **A → the `Op` decision → P → D**. Earlier: 🚦 **THE NEXT WORK IS SCOPED AND NOT STARTED (2026-08-20)** — "THE REMAINING WORK, SCOPED", near the top of the file. **X1** refuse `(pagesize N)` (a live wrong-module defect — it assembles and runs today), **X2** the `ModuleBuild` field-coverage sweep (T10a's open half, which is exactly what would have caught X1), **X3** a proposal's `Feature` gate is a track DELIVERABLE (15 members, none for the three new proposals — wazmrt shipped one unrefusable that way). Then **W** wide-arithmetic (1f/108s, best ratio, ~3 KB next door) → **P** custom-page-sizes (34f/7s, 🔒 the memory-safety one: every bounds check is `pages × page_size`, and one site left holding 65536 is a hole not a miss) → **M/A** threads (13f/18s, ⚠️ **triage before costing** — the equivalent entry next door was wrong twice over and not one of its 15 was a threads defect) → **D** custom-descriptors (64f/451s, largest and riskiest; **D1 `(ref (exact $t))` changes SUBTYPING** and carries a type-confusion soundness checkpoint — every cast arm needs a by-construction WRONG-ANSWER test, because an assertion count shipped a soundness hole next door). ⚠️ **436 of D's 451 skips and 99 of W's 108 are CASCADES** — a skip total over-counts as a size estimate. Then: 🆕 **2026-08-20, T13 DAY 2: F1–F7 CLOSED and S1/S2/S3/S6/S7 CLOSED — the 257 CORE spec files are at 0 failed / 0 skipped.** 63,333/172/1,024 → **63,807/112/584**. Everything left is proposal IMPLEMENTATION inside `proposals/`: custom-descriptors 64/451 (track D), custom-page-sizes 34/7 (P), threads 13/18 (M/A), wide-arithmetic 1/108 (W) — a scope decision, not a bug hunt. ⚠️⚠️ Two of the day's fixes were **wire-format divergences the spec suite could not see** (non-null abstract reftypes emitted as internal tags; `try_table`'s catch label off by one) — wasmtime refuses both. The F/S tables below are retained as the record of how the work was scoped; their numbers are the 08-19 baseline. Then: 🔒 **THE ORDER IS SET (owner, 2026-08-19): PHASE 1 = ALL 172 FAILURES, PHASE 2 = THE RESIDUAL SKIPS.** Borrowed from wazmrt and **probed, not adopted** — 739 of 930 cascade skips (79%) sit in files that also carry failures, so most of phase 2 clears itself; but the 191 that do not include **160 that are S1 alone**, across four files with zero failures between them, so **phase 2 is mandatory and re-measured**. 🆕 **2026-08-19 — THE SCOPED WORK-LIST, and it SUPERSEDES the track table.** Ranked by *assertions unblocked* now that the skip census makes the skip column attributable: **F1–F7** for the 172 failures (**F1 = accept-invalid, 32 core, ranked first on DIRECTION not count**) and **S1–S7** for the 1,024 skips (**928 are cascades**). ⚠️⚠️ **S1 — the externref/anyref bridge — is worth ~181 assertions and was invisible**: five unassemblable modules strand 176, and four of the five files have zero failures so the report never listed them. Three of the seven skip items ARE tracks D/W/L; three (S1, S2, S7) **no track covered**. Order: S1 → F1 → S2 → S3 → F2–F7 → S7 → D/W/P/M/A/L. 🆕 **2026-08-19 — three sections added, all borrowed method (`best-practices.md` §3A.2).** **T11 gains a HOW-to-measure block**: measure the floor first and report it (a spawn floor hid a 20–55× engine difference); quote the difference, not a load-dependent ratio; ⚠️⚠️ **a size number must be reproducible in the configuration it was recorded in — the never-measured rlib is an ARCHIVE, which measured four different sizes for one unchanged commit elsewhere**; bisect the ~5% regression; and **T11 holds a FIX task and a COMPARE task that must not share a queue**. Plus 📌 **the competitor has published head-to-head numbers first** — cdylib **222 KB vs 554 KB**, CLI **890 vs 684**, end-to-end a tie — which **does not close the gap, it raises its priority** (not parity-verified, and it disagrees with our own 493.5 KiB / 621 KiB). **T10a's gate AMENDED**: 🚨🚨 a round trip proves agreement with yourself, so add an external arm. **T10b added**: pin the three spellings of the proposal list. **T12z gains a row** (gate and typing rules must be ONE call) and **T12x is reshaped** for a world with no oracle — require agreement, record disagreements as observations. ⚠️ Status block re-audited (it had been five days stale). Then: 🔒 **ANCHOR CHANGE 2026-08-11 — the plan keeps its shape (finish T9 -> T10 -> T11 -> T12 -> 1.0) but T11 now DECIDES THE CONTEST rather than polishing it, carrying three promoted footnotes: the unattributed ~5%% steady regression, the NEVER-MEASURED rlib that rsxtk actually links, and no same-machine comparison to any competitor.** 🚦 **T12y + T12z added 2026-08-10.** T12z = the security-review METHOD: name an invariant, enumerate EVERY entry point in a table, check each — because both defects this week were "three of the four do X", never "nobody does X". T12y = ⚠️ **the oracle's sandbox-escape tests DO NOT RUN on this host** (the 4 in "489/493, 4 skipped" are the symlink traversal + resolver-fuzz tests; they self-skip because Windows denies native symlink creation, so the escape properties are UNVERIFIED here — enable Developer Mode before T12 and treat any resulting FAILURE as a finding the skip was hiding). Plus: **T9 IS IN PROGRESS — FIVE passes landed 2026-08-08** (memory imports, decoder strictness, the cross-store `InstanceId` fix, declared subtyping, type canonicalisation). **T10a ADDED 2026-08-08 — the EMITTER audit**: three "emits a different module than the text describes" defects in two passes, all found by accident, so T10a specifies a **round-trip property test** + a **`ModuleBuild` field-coverage sweep** rather than a read-through. **T9h ADDED 2026-08-08 — cross-module type identity via a `Store` type registry, approach APPROVED (wasmtime's SHAPE, our code; ~11 assertions)**. Earlier detail:: T9a#4's **memory half ✅** + `assert_unlinkable` + link-time import type checking, then **T9a#11 ✅ and most of #12 ◐** — the decoder-strictness cluster (seven checks; 99.0%, first time over 99). #12's remainder is the **text parser**, not the decoder. First pass (08-07): T9a #1/#2/#3 (+3 unlisted defects), **T9b size ✅ measured**, **T9c perf ✅ measured** (`examples/bench.rs`), **T9d licensing/docs ✅ done**. Open: T9a#4's **table** half (🚦 decision-gate), #5–#9, #11, #12, T9e `pin`, T9f tail calls. Original scoping (2026-08-06) — T9a correctness punch-list (12 items, each with its measured cost, ordered by value), T9b size, T9c perf, T9d licensing/docs, **T9h — cross-module type identity via a `Store` type registry (approach APPROVED 2026-08-08: wasmtime's SHAPE, our code; ~11 assertions)**, T9e `pin`, T9f tail calls (the last unimplemented IN-SCOPE proposal — 1.0 cannot be claimed without them), T9g scope confirmations (what to leave failing, and why the in-scope residual is far smaller than 738/3,075 suggests). **T10 (0.11) = bug hunt + code hygiene, now including T10a — the EMITTER audit (added 2026-08-08: three "emits a different module than the text describes" defects in two passes, all found by accident; T10a specifies a round-trip property test + a ModuleBuild field-coverage sweep instead of a read-through)**, **T11 (0.12) = optimization review** and **T12 (0.13) = security review** were added by the owner 2026-08-06; the ordering **measure → find → optimize → attack** is deliberate, and T12 records that `panic = "abort"` makes any reachable panic a host-process kill. Current status (**PORT phase; T0–T8 done, published through v0.9.0, oracle frozen @dadc727**) + the **conversion task list** T0–T12 (scaffold → types/reader → opcode → decode → validate → interp slices → text → wasi → C-ABI → hardening → bug hunt → optimization review → security review), parity-gated, with the 4 decision-gates inlined (**all now resolved**) and T9 scoped to what actually remains. Loader phases 1–4 |
| [known-issues.md](known-issues.md) | 🔴 **2026-09-20: the VALIDATOR REVIEW — EIGHT findings, and two of them were not in the validator.** A table's ELEMENT TYPE was read as a value type, so `(table (elem i64))` decoded, validated and **RAN**, handing the guest the engine's null sentinel — and `wasmrt wat` EMITTED such modules (8th T10a instance). Every 64-bit TABLE access truncated its index to 32 bits, so `call_indirect` at `2^32` **called slot 0 and returned its answer** where wasmtime traps. `return_call_indirect` typed its table index `i32` (both directions). Plus: a constant expression's operand stack capped at EIGHT (refused valid modules, with the wrong reason), an uninitialized non-null local readable in unreachable code, table64 and the table-initializer form each gated at one entry point of two, export-name uniqueness O(n²) (**128k exports = 11.9 s** vs wasm-tools' 0.13 s), and `--features` applied by ONE CLI path of three (a live swappability break — the sibling refuses the same command line). All CLOSED; conformance unchanged at 64,603 / 0 / 0, and **three of the eight came from sweeps, not from reading**. · 🔴 **2026-09-19: the DECODER REVIEW — EIGHT findings, all reproduced against wasm-tools BEFORE any change, and fixing one found a NINTH in the ASSEMBLER** (two were REJECT-VALID: a private `skip_const_expr` could not read `struct.new_desc` or `ref.null (exact …)`, fatal only at type index 11 where the stranded byte is `end`; five fields accepted their undefined values; `array.new_data`/`array.init_data` were missing from the data-count requirement in the decoder **and** the emitter, so `wasmrt wat` produced modules wasm-tools refuses). All CLOSED; conformance unchanged at 64,603 / 0 / 0, because none of it is reachable from the spec suite.** · 🔒 **2026-09-19: the WASI SUBSYSTEM REVIEW — TEN findings, one a SANDBOX ESCAPE** (`path_open` let the OS follow an unfollowed final symlink out of the preopen; the guard for it was computed and never read), three guest numbers that sized an allocation, six wrong answers reported as success. All fixed and pinned. Earlier: 🔎 **2026-09-19: a CODE REVIEW of the day's own work found SIX, five silent** — the environment wipe, the fail-open unreadable pin DB, `--verify` accepted-and-dropped in two spellings, `--` not forcing guest argv, and a conformance-gate blind spot. All fixed and pinned. Earlier: 🤝 **2026-09-19 (part 8): CLOSED, coordinated — the CLI exit status is the verdict (invalid module / no args / `wast` failures all exited 0; wazmrt exits 1; §2.3 was already AGREED).** Earlier: ✅ **2026-09-19 (part 7): value types in function bodies CLOSED — 20 invalid encodings were accepted, 3 valid block types refused (🆕, unlogged), `select (result (ref …))` undecodable; one reader now, wasmtime 48 passes the regression file whole. The two logged decoder entries are closed; other OPEN entries in the file are unrelated to it.** Earlier: ✅ **2026-09-19: the core-vs-custom-descriptors `br_on_cast` contradiction CLOSED (a runner defect — features per file) and flat-form GC instructions CLOSED; two 🟡 OPEN decoder accept-invalid defects remain (typed-`select`, block-type tags).** Earlier: ✅ **TWO ENTRIES CLOSED 2026-09-17.** `(pagesize N)` is refused by name (X1) — and the `elem` half of the bare-index segment gap **was not a false rejection, it assembled a DIFFERENT segment** (active → passive, one item → three), caught only because an `i32.const` cannot be a `funcref`. 🔴 **ONE ENTRY REOPENED**: the era-pinned `proposals/threads/` decision ("refresh the snapshot") is a **NO-OP** — the 08-20 upstream sync did not touch `proposals/threads/`, so the vendored copy already IS upstream and upstream is the stale one. Three surviving routes are listed; owner call. 🟡 **ONE NEW, and it is a CONTRACT SURFACE**: `wasmrt <file>` prints `validation FAILED` and **exits 0**, so a script using it as a validity gate passes every invalid module — found by falling into it (a probe table built on `wasmrt <file> && echo OK` reported four refused forms as accepted). **Not changed: exit codes are `interop.md` §0 and wazmrt leads.** Earlier: 🔴 **OPEN, AND IT IS THE FIRST ENTRY: `(pagesize N)` is parsed and SILENTLY DROPPED** — `(memory 1 (pagesize 1))` emits bytes identical to `(memory 1)`, so a 1-byte memory runs as 64 KiB and out-of-bounds accesses succeed. A wrong module, not a missing feature; wasmtime refuses the same source. Fix is X1: refuse the clause until Track P lands. Then: ✅ **2026-08-20 — THE FILE OPENS WITH A CLOSED LIST, and it must be read before anything below it.** S1, S2, S3, S6, S7 and F1–F7 are done; several entries further down state as a *deliberate decision* something that is now simply implemented (the externref bridge, imported tags, imported globals by value, legacy `delegate`, the three hierarchy bottoms, `anyfunc`). 🆕 **The one standing instruction that replaced them: run a format-level change through `wasmtime compile out.wasm`** — wasmrt emitted binaries no other engine could read whenever they held a non-null abstract reftype or a `try_table`. Historical: **OPEN 2026-08-19 — S1, the externref/anyref bridge (`any.convert_extern`/`extern.convert_any`), ~181 assertions across five files that had never appeared in a conformance report.** 🔒 **Fix the REPRESENTATION first**: an `externref` crosses the C ABI as a raw `uint64_t` while `ref_matches`' `Any` arm reads any non-null non-i31 reference as a **GC heap index** — same numeric space, so adding the arms first makes **host handle 2 read as GC object #2**. Tag it in the free high 64 bits of the `u128` `Value`. Issue tracker — ⬜ **OPEN 2026-08-19: the proposal list is spelled THREE times and nothing compares them** — `enum Feature` (core), the `u32` map `feature_of` (`wasmrt-capi/src/lib.rs:332`) and `wasmrt_feature_t` (`wasmrt.h:151`). ⚠️ `feature_of` matches on `u32` with a `_` arm, **so Rust's exhaustiveness check cannot fire**: a sixteenth proposal would compile, ship a header constant, and be unreachable from C **with no test failing**. Latent today (hand-verified consistent, `EVERY` pins the Rust side only); wazmrt shipped exactly this defect — a header advertising `TAIL_CALL = 14` against a C-ABI bound that stopped at 13. Fix = **T10b**. Also ⚠️⚠️ **THE START FUNCTION NEVER RAN (fixed 2026-08-08)**: `Module::start` was decoded, validated twice, assembled and printed by the CLI, and no code path ever called it (§4.5.5 step 11). A module initialized entirely by `(start $f)` returned defaults with **no error at any stage** — the worst silent-wrong-output defect the port has produced, and 10 of its 12 assertions had been failing for **five releases** in files literally named `start.wast`. **Triage read messages for a diagnosis; the cruder question — does the feature this file is NAMED for work at all? — was never asked.** Found by wiring trap backtraces and asking where an *instantiation* trap gets its frames. Also ✅ **trap backtraces DONE (T9a#7)** — the T8 bet paid off, real frames with **no ABI change**; ⚠️ its plumbing was not free by default (`pc: &mut usize` measured 3.6%% slower; a one-shot closure recovered it). Plus: ✅ **Decoder strictness DONE 2026-08-08** (T9a#11 + most of #12): seven decode-stage checks, **99.3%**, and the finding that #11/#12 were never two items but one theme in two halves (*wrong stage* and *over-acceptance*). ⚠️ **Section order is not id order.** A repeated section had been **silently replacing** the first; four of our own fixtures turned out to be malformed modules; and the const-expr sweep must key on segment **mode**, not on empty bytes. Cold start unchanged within noise once the redundant `Code::body` copy went. Also ✅ **T9a#4's memory half DONE 2026-08-08** (owner chose option 2: memories ship, tables stay refused), with the three non-obvious details — `module.memories` is the whole index space so only its tail is allocated; active data segments fork on the index so a later failure leaves no orphaned slots; limits matching compares **declared** types, so `Memory` carries its `min`. **Plus what unskipping `assert_unlinkable` found: imports were NEVER type-checked at link time** (silent-wrong-call), now fixed in the two places the type is known, with the equality-vs-subtyping trade **measured** rather than argued. ⚠️ **The obvious funcref packing is wrong — bits 32–63 collide with `I31_TAG`.** New open items: **no type canonicalisation (~40 assertions, the largest in-scope cluster left)**, host imports unchecked by construction, and malformed modules reaching the *link* stage. Also: ✅ **T9a first pass FIXED 6 defects (2026-08-07)**, three of which were not on any list: `Op::MemorySize` reading another instance's memory (mutation-verified fix), the `.wast` runner's failed-build redirect, `ref.null $t` (**plus a silent-wrong-value bug in the same arm — `nofunc` assembled as `func`**), the inline table-elem shorthand, an **undecodable concrete-ref block type**, and **`br_table` label typing wrong in two directions**. ⚠️ **`ref.null`'s logged cause was WRONG** — it did not move `br_table.wast`, which needed all four assembler/decoder/validator fixes; treat every remaining cost figure as a hypothesis. 🚦 **NEW DECISION-GATE: T9a#4 is not plumbing** — a `funcref` carries no instance identity, so an imported *table* would dispatch to the wrong function; imported *memories* are safe, imported tables need the value encoding decided. Plus: the **2 silent-wrong-output defects fixed at T8** (dropped table initializers; element-segment form 4 rewriting a type) **and the third-order lesson**: a check that is harmless "because the case cannot occur" becomes a bug the moment it can; what T8 closed and what it left open; the **ranked pre-T8 punch list** (2026-08-05), the **5 open defects the wasmtk corpus surfaced** (`ref.null $T` unassemblable · 2 validator gaps the oracle passes · no trap backtrace · an unconditional `data_count` section), the fixed shared-store and literal/text defect write-ups, the port's intentional divergences (single-threaded atomics, `delegate` rejected everywhere, the debug-build stack-depth finding), the wazmrt residuals (#17/#18/#23), and the open decisions |
...
- 🆕 **The three sweeps the validator review added (2026-09-20)**, all differential against an outside
  ...
  - `scripts/feature-gate-sweep.ts` — every proposal, disabled, against a module that uses it; wasmrt
    and wasm-tools must both refuse, and the all-enabled control must be accepted by both. Found
    table64 gated for memories only (X3's shape, third instance).
  - `scripts/index-type-sweep.ts` — the 64-bit axis: 23 table/memory instructions × {i64 accepted,
    i32 refused}, then 10 of them executed past `2^32` against **wasmtime**, where both must trap.
    Found `return_call_indirect` typing its index `i32`, and six table instructions truncating a
    64-bit index at run time.
  - `scripts/differential-fuzz.ts` — `wasm-tools smith` modules (valid by construction → any wasmrt
    refusal is a defect) plus one-byte mutations kept only when wasm-tools calls them invalid (→ any
    wasmrt acceptance is a defect). Found the constant-expression operand cap and a table whose
    element type is `i64`. ⚠️ Judge with **wasmrt's** proposal set, not `-f all`.

...
order: **track A** (`custom/`, 20 skips of runner work) → the import type-use check → **track P** →
**track D** (now 98% of what remains, and unblocked — `Op` has 0xEB free tags).

Working tree is **ahead of the published v0.9.0**. Suite **64,068 / 66 / 549 — 99.9% of 64,134** over
**288** files; **503 workspace tests**; clippy clean; C-ABI gate PASSED (74 symbols); `.wat` corpus
**528/532** ⚠️ *(not 530 — the corpus loop keyed on the CLI's exit status, which is 0 even when validation fails; reading the verdict instead exposed two pre-existing failures. All four are corpus defects wasmtime also refuses.)*; **no file lost a pass** in any pass, across T9's eighteen and T13's three days.
⚠️ **Miri NOT RUN on day 3** — not installed on this host, so the 28/28 on record is not re-verified.
...
refusals). What is queued, in order: **track A** — the `custom/` directory's two unhandled `assert_*`

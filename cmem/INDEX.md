...
Working tree is **ahead of the published v0.9.0**. Suite **61,724 / 554 / 2,466 — 99.1%** of 62,290
...
three passes — the check that matters whenever skips convert into verdicts.
...
🔧 **Still open: cross-module type identity (~11).** A canonical id is module-local; self-contained keys risk
exponential blowup. The answer is an **engine-level type registry on `Store`** (what wasmtime does) — a

...
## ◐ T13 CONFORMANCE CLEAR-OUT IN PROGRESS — day 2 landed 2026-08-20, unreleased

Working tree is **ahead of the published v0.9.0**. Suite **63,807 / 112 / 584 — 99.8%**;
**490 workspace tests**; clippy clean; **no file lost a pass** in any pass, across T9's eighteen and
T13's two days.

🎯 **The 257 CORE spec files are at 0 failed / 0 skipped.** Every remaining number is inside
`proposals/`: custom-descriptors 64/451, custom-page-sizes 34/7, threads 13/18, wide-arithmetic 1/108.
Day 2 closed **F1–F7** (58 core failures → 0) and **S1, S2, S3, S6, S7**. What is left is proposal
IMPLEMENTATION — tracks **D**, **P**, **M/A**, **W** — not defect repair, and it is a scope decision
rather than a bug hunt. Detail in [roadmap.md](roadmap.md); numbers in [testing.md](testing.md).
...
🚦 **NEXT WORK IS SCOPED AND NOT STARTED** — `roadmap.md`, "THE REMAINING WORK, SCOPED
2026-08-20". Three cross-cutting items (**X1** refuse `(pagesize N)`, **X2** the `ModuleBuild`
field-coverage sweep, **X3** a proposal's `Feature` gate is a track deliverable), then four tracks
ranked **W → P → M/A → D**. ⚠️ Two predictions are recorded up front so honest movement is not read
as regression: **passes go DOWN when `exact` lands** (a parse gap currently scores as a correct
rejection, so ~19 of `exact.wast`'s passes are false) and **skips RISE when X1 lands**.

...
## ◐ T9 IN PROGRESS (2026-08-08) — five passes landed today, unreleased

Working tree is **ahead of the published v0.9.0**. Suite **61,724 / 554 / 2,466 — 99.1%** of 62,278
adjudicated — **the first time over 99%**; **397 tests** (371 core + 26 capi); clippy, all four surfaces,
the C-ABI gate (74/74 + `c_smoke`) and Miri (26/26) green. **No file lost a single pass** in any of the
five passes — the check that matters whenever skips convert into verdicts.
...
🔧 **Cross-module type identity (~11) — now scoped as T9h, approach APPROVED by the owner 2026-08-08:** an
**engine-level type registry on `Store`**, interning rec groups as modules join so matching compares ids
rather than structures. A canonical id is module-local, and self-contained keys risk exponential blowup on
chained groups — which is a DoS surface on the untrusted path, so the registry is the right answer.
**wasmtime's SHAPE, our code**, and the third application of that standing rule, not a new policy — see the
🔒 entry in [design-decisions.md](design-decisions.md). **The Component Ledger stays empty:** the "evaluate a
reference project" trigger below requires an entry for *copying or porting code*, and reading an
architecture is explicitly free. wasmrt remains 100% original Rust.
...
**Still open in T9:** T9a #4's **table** half (gated), #5, #6, #7, #8, #9, and #12's **text-parser**
remainder (`func.wast` 21, now in `wat.rs` not `module.rs`) · **T9e `pin`** (still a stub — a build
performs **no** authenticity check) · **T9f tail calls** (1.0 cannot be claimed without them).

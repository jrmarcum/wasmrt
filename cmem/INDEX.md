...
## ◐ T13 CONFORMANCE CLEAR-OUT IN PROGRESS — day 1 landed 2026-08-19, unreleased

Working tree is **ahead of the published v0.9.0**. Suite **63,333 / 172 / 1,024 — 99.7%** of **63,505**
adjudicated over 284 files; **484 tests**; clippy clean; **no file lost a pass** in any pass, across T9's
eighteen and T13's day-1 run of ~20. ⚠️ the **`.wat` corpus figure is UNVERIFIED** — its denominator
moved (532 files today), see [testing.md](testing.md).

**Day 1 moved 62,238 / 378 / 2,038 → 63,333 / 172 / 1,024**: +1,095 passes, **−206 failures**, −1,014
skips. ⚠️ *Adjudicated grew because skips became verdicts, so the percentage understates it — read the
three numbers, never the rate alone.* Landed: the six missing **WasmGC array bulk ops**, **table64 end
to end**, the assembler scoring split, §6.5.2 label repetition, typed `select`, `br_on_non_null`,
elem/binary/instance to zero, the relaxed-SIMD `either` form + NaN quieting, a named local aliasing a
parameter, SIMD lane sign, duplicate `$name`s, an implicit type use reusing a rec-group member,
`(register "M")` after a named module, and `memory.init` reading its operands **backwards**.

🔬 **The day ended on the INSTRUMENT, and that is the finding to carry.** The runner listed a file only
when it had failures — so **234 skips lived in files that printed nothing**, and skips had no recorded
reason at all. Fixed, and the census says **928 of 1,024 skips (91%) are CASCADES** behind a handful of
roots; only **96** are real gaps, resolving to **seven items**. The remaining work is scoped in
[roadmap.md](roadmap.md) as **F1–F7** (172 failures) and **S1–S7** (1,024 skips), ranked by *assertions
unblocked*.

🔒 **ORDER: ALL FAILURES FIRST, THEN THE SKIPS (owner, 2026-08-19).** wazmrt found that fixing the
failures cleared a lot of the skips; **probed here rather than adopted**, and it holds — **739 of 930
cascade skips (79%) sit in files that also carry failures**, so most of the skip work is downstream of
the failure work. `exact-casts.wast` is 3 failures holding 108 skips with zero capability gaps.
🎓 **Read the skip column to RANK; work the failure column to FIX**
([best-practices.md](best-practices.md) §5.5a — the inverse of §5.6, and both true because they answer
different questions).
...
`ref_cast`, `br_on_cast` and `br_on_cast_fail`, which have **zero failures between them**, and none of
its five files had ever appeared in a conformance report. See [best-practices.md](best-practices.md) §5.6.

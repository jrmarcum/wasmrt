...
| Axis | Standing | Gap |
| --- | --- | --- |
| **Canonical** | **99.4%** of the official spec testsuite (62,238 / 378 / 2,038 over 284 files); the 533-file `.wat` corpus is clean through assemble→decode→validate, and every CLI command that takes a module accepts `.wat` directly; **every proposal in the scope list now runs, tail calls included** | ✅ **Tail calls landed 2026-08-14 (T9f)** — previously the one gap; `return_call`/`return_call_indirect` are not in the opcode table (`return_call_ref` is, via function-references). Consequently the C ABI has **no tail-call feature flag**: a toggle that gates nothing is worse than none. **That removes the last feature blocker on 1.0.** |

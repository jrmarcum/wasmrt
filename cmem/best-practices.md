...
## §4.4 — **A PROPERTY ASSERTED IN A DOC COMMENT IS NOT A CHECK** (2026-09-17)

`features::op_feature` maps an opcode to the proposal that gates it. Its doc comment said:

> **One authority.** Every gate in the validator reads this table, so an instruction cannot be gated in
> one place and forgotten in another — and a new opcode added to `opcode` shows up here as an explicit
> decision rather than silently defaulting to "always allowed", **because the match below is exhaustive
> over `Op`**.

**The match ends in `_ => return None`.** It is not exhaustive and never was, so a new opcode does not
show up as a decision — it silently becomes "WebAssembly 1.0, cannot be gated", which for a proposal
instruction means **UNREFUSABLE**. Track W's four wide-arithmetic ops landed exactly that way: wired
through decoder, assembler, validator and interpreter, and switchable by no flag. Measured afterwards as
**182 ungated ops without the gate arm, 178 with it.**

🎓 **The comment is worse than nothing, because it is the reason nobody looks.** A reviewer who reads
"exhaustive over `Op`" and then adds an op has been told the compiler will catch the omission. `Op` had
248 of 256 discriminants assigned; nobody was going to count.

**What to do instead — pin the property with DATA:**
* Enumerate the thing (`Op::ALL`, generated by the same macro that defines the enum, so it cannot drift).
* Assert the *set* or the *count* of items that fall through. Adding one then either satisfies the
  property or produces a visible, deliberate diff.
* Where the safe direction is asymmetric, say which: an item wrongly *in* the ungated set is
  unrefusable, which is the direction that cannot be noticed afterwards.

⚠️ **The same shape, two hours later, in the same feature.** The proposal list has **three** spellings —
the Rust `Feature` enum, `Features`' struct fields, and `wasmrt_feature_t` + `feature_of` in the C ABI.
`WideArithmetic` reached the first two. Nothing compared them, so it was switchable in-process and **not
through the C ABI** — gated at one entry point of two, which is precisely what X3's "gated at BOTH module
entry points" exists to prevent. T10b had predicted this and the prediction was not yet a test. It is
now: `set`/`has` must round-trip for every member, the count is fixed, and the newest feature must be
LAST because the C integers are frozen.

🔒 **Corollary for `cmem/` itself.** This file, `design-decisions.md` and every `///` block are prose.
None of them can fail a build. When a note claims a property the code enforces, either the enforcement is
findable as a test — name it in the note — or the note is a hypothesis, and §1's rule applies to it like
any other.

## §3.12 — **AN UNKNOWN SPELLING IS NOT AUTOMATICALLY REFUSED — IT CAN BE RE-READ** (2026-09-17)

The bare-index segment spellings, `(data 0 (i32.const 0) "x")` and `(elem 0 (i32.const 1) $f)`, are
valid and both parsers read only the parenthesised `(memory 0)` / `(table 0)` form. The two halves failed
**differently**, and the difference is the lesson:

| | what happened |
| --- | --- |
| `data` | the `0` fell through to the datastring loop → `BadForm`. **A false rejection** — the safe direction, and still wrong. |
| `elem` | ⚠️⚠️ **the module ASSEMBLED, and it was a different module.** Nothing consumed the `0`, so the offset branch did not fire either: an **active** segment writing one funcref at index 1 became a **passive** segment of three items — `ref.func 0`, the offset expression itself, and `ref.func $f`. |

It surfaced only because an `i32.const` cannot be a `funcref`, and then as the validator's `TypeMismatch`
— an error naming neither the stage nor the thing that was wrong. **Had the items happened to type-check,
this was a silently wrong module.**

🎓 **The intuition to give up: "we don't parse it, so we reject it."** A parser that does not recognise a
token at a position does not stop — it keeps going and the token lands wherever the next rule looks. That
is §3A.2's "a synthetic tag in a real space eventually means something else", and §3.11a's "a rejection
routed to code that does not recognise it lands in the wrong bucket", arriving from a third direction.

**So a test for a newly-accepted spelling asserts the MEANING, not acceptance.** `assemble()` returning
`Ok` proves nothing about what was built; the regression test here checks the decoded segment's mode is
`Active` and that it holds exactly one item. That is the same rule as §3.8b — hand the bytes to a second
reader — applied to your own decoder.
...
X1 landed squarely in it. `custom-page-sizes/memory_max.wast` went **2 passed → 0 passed / 6 skipped**
and the gate printed *"no file lost a pass"*. Two files were reported; four were affected. Printing a row
for every file — 288 rows — makes the join total, and the same gate then found all four.

🎓 **A gate can be sound on the rows it has and blind to the rows it does not have.** Ask what the
instrument *omits*, not only what it gets wrong: the two earlier holes were about comparing rows badly,
this one was about a row that was never written. All three had the same symptom — a green verdict.

⚠️ **And it happens in throwaway tooling too, within the same session.** A shell probe table comparing
segment spellings against wasmtime used `wasmrt <file> && echo OK` as its oracle. `wasmrt <file>` prints
`validation FAILED` and **exits 0**, so the table reported four forms as accepted that the engine was
refusing. *A gate you build for yourself, five minutes before trusting it, is still a gate.* Check that
it can fail before reading its output.

## §1.7 — **WHEN THE CORPUS CAN MOVE, THE DENOMINATOR IS A MEASUREMENT** (2026-09-17)

Day 3 opened by re-running the suite rather than quoting day 2's **63,807 / 112 / 584 over 284 files**.
It read **63,963 / 112 / 604 over 288** — wasmtk had synced the vendored testsuite to upstream on
2026-08-20 at 18:34, hours after those numbers were written: **+4 files, +156 passes, +20 skips.**

⚠️⚠️ **The failure total was 112 on both sides, by coincidence** — custom-descriptors +1,
custom-page-sizes −2, a new `custom/` directory +1. An engineer who checked only the headline would have
concluded the corpus had not changed, and every later delta would have been measured against a mix of two
corpora.

🎓 Three consequences worth keeping:
* **An unchanged total is not evidence that nothing changed.** Only the per-file join can say that, which
  is the same reason the gate is per-file.
* **A vendored corpus is an input under someone else's control.** Re-measure it at the start of a session,
  and record the file count beside the assertion counts — 288 vs 284 was the visible half.
* **New corpus files can arrive with no track covering them.** The `custom/` directory brought 20 skips
  of unhandled `assert_*_custom` commands that no T13 track had scoped, because it did not exist when
  they were scoped.

## §8.4 — **VERIFYING THE PREMISE OF AN OPTION IS NOT VERIFYING THE OPTION** (2026-09-17)

The era-pinned `proposals/threads/` assertions were decided on 2026-08-20: **refresh the vendored
snapshot**, chosen over an explained baseline entry and over deleting the file, and chosen *after* the
premise was checked against data already on disk — the core `imports.wast` in the same checkout has 0
"multiple memories" assertions, the threads snapshot has 6. That check was real and the conclusion "the
snapshot is behind" is true.

**It is also not the claim the option needed.** "Behind CORE" is not "a newer snapshot exists". The sync
to upstream `65a43d2e` **did not touch `proposals/threads/` at all** — upstream's own threads snapshot
carries the same stale assertions — so there is nothing to refresh to, and the decision is a **no-op**.

🎓 **Check the premise the ACTION depends on, not the premise the diagnosis depends on.** The diagnosis
("the test is old, the engine is right") was correct and remains correct. The action assumed a second

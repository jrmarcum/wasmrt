...
| **T13-T** | 64-bit tables | ~185 | ◐ **FIRST SLICE DONE 2026-08-19** — types, assembler and validator typing. Suite **62,435/378/1,825 → 62,566/342/1,696**, rate **99.4% → 99.5%**, no file lost a pass; `table_copy64` 0→1,654 passing, `table_init64` 0→745. ✅ Confirms T13-0: **`BadNumber` WAS table64** and the cluster is gone. Still open: `table_get64` 0/1/10, `table_set64` 0/1/18, `float_memory64` 0/6/84 — the interpreter side. | `table_copy64` 22, `table_init64` 93, `table_fill64` 70, `float_memory64` 84. Reverses the "tables stay 32-bit" invariant. The memory64 work is the template — *a memory carries its own index type* already exists. |
